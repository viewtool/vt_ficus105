...
int main(int argc, char* argv[])
{
    ...
    for(j=0;j<20;j++)
    {
        ...
        ret = VII_InitI2C(VII_USBI2C, 0,num[j], &I2C_Config);
        if (ret != ERR_SUCCESS)
        {
        ...
        ret = VII_TimeConfig(VII_USBI2C, 0, num[j], &I2C_TimeConfig);
        if (ret != ERR_SUCCESS)
        {
        ...
        ret = VII_WriteBytes(VII_USBI2C, 0, num[j], 0xA0, 0x00, write_buffer, 8);
        if (ret != ERR_SUCCESS)
        {
        ...
        ret = VII_ReadBytes(VII_USBI2C, 0, num[j], 0xA0, 0x00, read_buffer, 8);
        if (ret != ERR_SUCCESS)
        {
